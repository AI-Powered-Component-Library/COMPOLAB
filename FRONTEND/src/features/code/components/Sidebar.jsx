...
const Sidebar = ({ components = [], setSelected }) => {
    const [active, setActive] = useState(null)

    useEffect(() => { setActive(components[0]?._id) }, [components])
    ...
    return (
        <aside className="relative w-70 border-r border-violet-950/90 flex flex-col gap-1 px-3 flex-shrink-0 select-none">
            ...
            {/* header */}
            <div className="px-4 pb-4 mb-1 border-b border-violet-500/10">
                <div className="flex items-center gap-2">

                    <span className="text-base  uppercase text-indigo-400 font-semibold">
                        components
                    </span>
                </div>
